# RFC Graph Visualizer — Aggiornamenti 4

**Stato:** rispetto al documento precedente, backend e JSON restano invariati: le novità di questo documento riguardano soprattutto il rendering del grafo 3D in `GraphCanvasComponent`, con in aggiunta una modifica al filtro per working group in `DraftTimelineComponent` (punto 9.5). Primo, è stata aggiunta una **forza di clustering basata sulla topologia del grafo** (rilevamento community via Label Propagation + una forza D3 custom che separa i cluster nello spazio, punto 8.3): i nodi non si dispongono più solo in base a charge/link/collide, ma tendono a raggrupparsi visivamente per vicinanza strutturale (Updates/Obsoletes), con un margine minimo imposto tra un gruppo e l'altro. Secondo, il raggio di collisione (`collisionRadiusFor`) è stato ridotto da un moltiplicatore di 4.5× a 2.2× rispetto al raggio visivo del nodo: quel valore, troppo grande rispetto alla sfera realmente renderizzata, era la causa di archi visivamente troppo lunghi (la collisione, imponendo una distanza minima tra i centri superiore alla distanza target del link, vinceva sempre sul link stesso); le iterazioni di collisione per tick sono state alzate da 2 a 3 per compensare l'overlap residuo introdotto dalla compressione più marcata dovuta al clustering. Terzo, il crash intermittente `"can't access property 'tick', e.layout is undefined"` nella libreria di rendering 3D — una race condition tra l'avvio del render loop e l'inizializzazione interna dello stato di layout — è stato **risolto**: il grafo viene ora creato con `pauseAnimation()` come prima chiamata, e riattivato con `resumeAnimation()` solo dopo un margine di sicurezza via `setTimeout`, così il render loop non parte mai prima che lo stato interno della libreria sia pronto. Infine, nella vista timeline draft il filtro per working group ora **rimuove dal disegno** i documenti non corrispondenti invece di attenuarli in grigio semitrasparente (punto 9.5): i draft esclusi non vengono più disegnati né sono cliccabili, e la pila si ricompatta. Questo documento riporta per intero, come il precedente, tutta la struttura del sistema — non solo queste novità — per restare autosufficiente.

**Nota di lettura:** questo documento è autosufficiente — ogni campo, componente e scelta implementativa è spiegato qui per intero. Non è necessario aver letto `aggiornamenti_e_proposte_1.md`, `aggiornamenti_e_proposte_2.md` o `aggiornamenti_e_proposte_3.md` per seguirlo: i richiami ai "documenti precedenti" servono solo a tracciare cosa è cambiato rispetto ai piani/proposte precedenti, e riportano già il contesto necessario a capirli senza dover andare a controllare le versioni precedenti.
...
La disciplina definitivo/transitorio (punto 1.1) copre la stragrande maggioranza dei casi, ma restano alcune situazioni note, marginali, che il codice attuale gestisce con scelte di compromesso:

* **Persistenza dei metadati (RFC vs. Draft):**
  * *Per gli RFC pubblicati:* una volta arricchiti, non vengono mai più riconsiderati. Il processo di `run_enrich()` utilizza un approccio puramente incrementale (`n["id"] not in result_nodes`): una volta risolti layer e working group, il dato è considerato definitivo. Questo garantisce performance elevate, ma significa che correzioni su Datatracker o modifiche ai `MANUAL_LAYER_OVERRIDES` non si propagheranno mai agli RFC già elaborati — e il blocco non è aggirabile nemmeno con `--force`, perché quel flag resetta solo lo stato (`enriched_ids`/`excluded_ids`), non `result_nodes`, che viene comunque ricaricato dal dataset già esistente.
  * *Per gli Internet-Draft (attivi/scaduti):* la gestione è duplice. Il ciclo di vita (es. transizioni verso `dead` o `repl`) è monitorato periodicamente tramite `recheck_active_drafts()`. I metadati specifici (anno e abstract) sono invece gestiti da `draft_metadata_enricher.py`, la cui `needs_enrichment()` guarda direttamente i campi del nodo (non lo stato salvato): un draft con `url` e `year` già valorizzati non viene più ritoccato, a prescindere da `--force`. Il flag serve solo a resettare `processed_ids`, rimettendo in gioco i nodi rimasti bloccati con `year is None` a causa di un esito definitivo-ma-vuoto (es. 404 su Datatracker, o campo `time` assente/malformato) — non comporta quindi la perdita dell'incrementalità sull'intero dataset, ma solo il ricalcolo del sottoinsieme di draft con metadati ancora mancanti.

* **Gestione namespace XML:** un file XML in input con un namespace diverso da quello atteso non genera errore. Il parser cerca elementi con il namespace fisso `https://www.rfc-editor.org/rfc-index`; se un file passato con `--offline` dichiara un namespace o una struttura non conforme all'indice IETF reale, `root.findall(...)` restituisce zero risultati. Lo script logga "Entry totali parsate: 0" e produce un dataset vuoto, senza sollevare un'eccezione che segnali esplicitamente l'incompatibilità del formato.

* **Euristica di `compute_impact_scores()`:** la funzione non implementa un PageRank puro. Il termine di "authority boost" (proporzionale al numero grezzo di archi entranti, punto 8.1) viene sommato direttamente al rank ad ogni iterazione, rompendo l'invarianza per cui la somma dei rank dovrebbe restare costante tra le iterazioni. È una scelta euristica dichiarata nel codice, non un bug, ma va tenuta in considerazione qualora si volesse confrontare o riprodurre i punteggi con implementazioni di PageRank standard.
...
Il layout è guidato da quattro forze (charge/repulsione, link/attrazione lungo gli archi, collide/anti-sovrapposizione, più una nuova forza di clustering, vedi 8.3.1), con alcuni accorgimenti specifici per un dataset di ~9.800 nodi:
...
- **Collisione (`forceCollide`) con raggio condiviso `collisionRadiusFor`**: `radiusFor(impact) * 2.2 + NODE_GAP / 2`, limitata a **3 iterazioni** per tick (alzate da 2). Il moltiplicatore è stato **ridotto da 4.5 a 2.2**: il valore precedente, molto più grande della sfera realmente renderizzata (quella usata da `nodeVal`, punto 8.2), imponeva una distanza minima tra i *centri* di due nodi collegati superiore alla distanza target del link — la collisione vinceva sempre sul link, e il risultato visivo erano archi anomalamente lunghi anche a `link.distance` basso. Con il moltiplicatore ridotto, la somma dei raggi di collisione di due nodi collegati può davvero scendere sotto la distanza voluta dal link, che quindi torna a poter avvicinare i nodi come inteso. Le iterazioni sono state alzate da 2 a 3 per compensare l'overlap residuo introdotto dalla forza di clustering (8.3.1), che ora comprime i nodi più vicini al centroide del proprio gruppo.
- **Passaggio finale deterministico di pulizia (`resolveAllCollisions`)**: la simulazione fisica converge in un tempo fisso (`cooldownTicks`/`cooldownTime`) ma non garantisce che *ogni* coppia di nodi vicini abbia risolto l'overlap in quel budget, specialmente in zone dense. Dopo il primo `onEngineStop`, un passaggio separato — con una griglia spaziale (celle di lato pari al doppio del raggio di collisione massimo, così bastano le 26 celle adiacenti per non perdere coppie in collisione, invece di un confronto O(n²) tra tutti i nodi) — allontana iterativamente ogni coppia ancora sovrapposta finché non ne resta nessuna (o fino a un tetto di sicurezza di 40 passate), usando lo stesso `collisionRadiusFor` di cui sopra (un'unica definizione condivisa, non due formule che potrebbero disallinearsi). Solo **dopo** questo passaggio il grafo viene rivelato all'utente (`graphReady`): così il risultato finale ha overlap zero garantito, indipendentemente da quanto ha girato la fisica prima.
...
- **Race condition al setup, ora prevenuta esplicitamente**: `3d-force-graph` avvia il proprio render loop (`requestAnimationFrame` → `tickFrame` → `layout.tick()`) non appena l'istanza viene creata, ma lo stato interno che quel loop legge (`state.layout`) viene valorizzato dal ciclo di aggiornamento di Kapsule con un `debounce(fn, 1)`, non in modo sincrono — un `.graphData(...)` incatenato subito dopo la creazione poteva quindi far scattare un tick prima che `state.layout` esistesse, con il crash `"can't access property 'tick', e.layout is undefined"`. Il grafo viene ora costruito con `.pauseAnimation()` come **prima chiamata concatenata**, che blocca il render loop finché non lo si riattiva esplicitamente; `resumeAnimation()` viene richiamato solo dopo un `setTimeout` con margine di sicurezza (50ms, ben oltre l'1ms del debounce interno), sia a fine `setupGraph()` sia dopo un `d3ReheatSimulation()` su dati nuovi. La race è eliminata alla radice invece che mitigata: il render loop semplicemente non parte mai prima che lo stato interno sia pronto.

#### 8.3.1 Forza di clustering: raggruppamento visivo per community

Novità di questo documento: oltre alle tre forze standard, il layout ora include una **quarta forza custom** (`forceCluster`, registrata come `d3Force('cluster', ...)`) che raggruppa spazialmente i nodi in base a "community" rilevate dalla sola topologia del grafo — non da attributi come `layer` o `working_group`, che restano riservati a colore/filtri (punti 8.4, 8.6).

- **Rilevamento delle community (`detectCommunities`)**: implementa il **Label Propagation Algorithm (LPA)**. Ogni nodo parte con un'etichetta propria (il proprio id); ad ogni iterazione adotta l'etichetta più frequente tra i vicini raggiunti da un arco Updates/Obsoletes (a parità di frequenza, vince l'etichetta lessicograficamente minore, per determinismo); dopo poche iterazioni (tetto di sicurezza a 8) le etichette convergono in gruppi di nodi densamente interconnessi. I nodi isolati (senza archi) restano ciascuno nella propria etichetta, invece di finire ammassati in un unico gruppo residuo. Il calcolo gira **una sola volta al caricamento del dataset** (non ad ogni tick), quindi il suo costo — O(iterazioni × archi) — resta trascurabile anche su ~9.800 nodi.
- **La forza vera e propria (`forceCluster`) fa due cose per ogni tick**: (1) **coesione** — tira ogni nodo verso il centroide della propria community; (2) **separazione netta tra cluster** — spinge i centroidi di community *diverse* lontani l'uno dall'altro finché la loro distanza non supera una soglia minima (`minSeparation` più un raggio stimato per ciascuna community, dal massimo tra una stima basata sul conteggio dei membri e la dispersione reale misurata). Questa seconda parte è quella che rende la separazione un vincolo imposto attivamente, non un effetto collaterale casuale del charge generico: due cluster hanno **sempre** un vuoto visibile tra loro, non solo "in media". Le community con meno di 4 membri non partecipano al confronto O(k²) a coppie tra centroidi (altrimenti k coinciderebbe quasi con il numero totale di nodi), ma restano comunque bersaglio della repulsione dei cluster grandi, così anche i nodi isolati o i mini-gruppi intrappolati dentro un cluster grande ne vengono spinti fuori. Lo spostamento imposto a un singolo cluster in un singolo tick è limitato (clamp a metà di `minSeparation`) per evitare scatti visivi quando più cluster si respingono contemporaneamente.
- **Effetto visivo pratico**: il grafo, invece di disporsi in una nuvola indifferenziata guidata solo da charge/link/collide, mostra ora addensamenti visivamente separati che corrispondono a gruppi di RFC storicamente collegati tra loro da Updates/Obsoletes — un livello di struttura aggiuntivo rispetto a colore (layer) e dimensione (impact score), utile in particolare al profilo "chi lavora dentro l'IETF" del punto 0, per individuare a colpo d'occhio famiglie di documenti correlati senza dover cliccare nodo per nodo.
...
- **Overlay di caricamento** con messaggi progressivi temporizzati ("Caricamento grafo RFC…" → "Assestamento delle forze…" a 3s → "Quasi fatto…" a 6.5s) e una rete di sicurezza a 10s che, se per qualche motivo `onEngineStop` non dovesse mai scattare (versioni diverse della libreria, dataset patologico, o macchine più lente dove la fisica non converge entro 10s), richiama comunque la stessa pulizia (`resolveAllCollisions` + `captureSettledLayout`) prima di rivelare il grafo.
...
- **Filtro per working group**, con la stessa logica di ricerca testuale della vista a grafo, ma un comportamento visivo diverso da quello: qui i documenti non corrispondenti al filtro selezionato vengono **rimossi dal disegno**, non attenuati. `rebuildVisibleIdsCache()` ricalcola, per ogni colonna-anno (e per il bucket "n.d."), la sola sotto-pila di id che soddisfano il filtro; `drawColumn`/`hitTest` leggono sempre questa pila filtrata (`visibleIdsForYear`) invece di quella completa, quindi i draft esclusi non vengono proprio disegnati né sono cliccabili, e la pila restante si ricompatta (nessuno spazio vuoto lasciato dagli elementi esclusi) — a differenza del punto 8.4 per il grafo 3D, dove il filtro attenua (colore grigio semitrasparente + raggio ridotto) senza mai rimuovere nulla dalla simulazione. Ogni voce del filtro mostra comunque **quanti documenti** appartengono a quel gruppo (`workingGroupCounts`, calcolato una sola volta a fine caricamento da `computeWorkingGroupCounts()`, con la stessa logica di fallback su `NO_WORKING_GROUP` usata per disegnare le colonne). Il contatore in toolbar riflette la stessa granularità: quando un filtro è attivo mostra sia il numero di documenti coperti dal filtro sia il totale (`filteredCount() / totalCount()`), invece del solo totale.
...
Riepilogo puntuale delle differenze tra quanto proposto/segnalato nei documenti precedenti e quanto risulta oggi dal codice. Le cinque voci seguenti sono le novità di **questo** documento rispetto al documento 3 (le voci successive erano già presenti in quello):

- **Forza di clustering per community topologiche** (novità di questo documento, punto 8.3.1): **aggiunta**, non presente nei documenti precedenti. Il layout del grafo 3D ora include, oltre a charge/link/collide, una quarta forza D3 custom che raggruppa i nodi per community rilevate via Label Propagation sulla sola topologia Updates/Obsoletes, con un margine minimo imposto tra cluster diversi.
- **Raggio di collisione e archi troppo lunghi** (novità di questo documento, punto 8.3): **corretto** — il moltiplicatore di `collisionRadiusFor` è sceso da 4.5× a 2.2× il raggio visivo del nodo, eliminando la causa per cui la collisione vinceva sempre sulla distanza voluta dal link. Le iterazioni di collisione per tick sono salite da 2 a 3 per compensare l'overlap aggiuntivo introdotto dal clustering.
- **Crash `"e.layout is undefined"` nel grafo 3D** (novità di questo documento, punto 8.3): **risolto**. La causa era una race condition tra l'avvio del render loop di `3d-force-graph` e la valorizzazione (asincrona/debounced) del suo stato interno di layout; risolta bloccando il render loop con `pauseAnimation()` alla creazione dell'istanza e riattivandolo con `resumeAnimation()` solo dopo un margine di sicurezza via `setTimeout`.
- **Restyling di coerenza tra le due viste** (novità di questo documento): tipografia, pulsanti e spaziature sono stati riallineati tra `GraphCanvasComponent` e `DraftTimelineComponent`; lo sfondo scuro/spaziale del grafo e quello bianco della timeline restano una scelta di tema deliberatamente distinta, non toccata dal restyling. In aggiunta, i nodi del layer Network nel grafo 3D presentano ora una nuova colorazione (sempre colorblind-safe).
- **Filtro per working group nella timeline draft** (novità di questo documento, punto 9.5): **cambiato** — non attenua più in grigio semitrasparente i documenti non corrispondenti al filtro, come nella vista a grafo 3D, ma li **rimuove dal disegno**: `rebuildVisibleIdsCache()` ricalcola per ogni colonna-anno la sola sotto-pila di id che soddisfano il filtro, e `drawColumn`/`hitTest` leggono sempre quella pila filtrata, quindi i draft esclusi non vengono disegnati né sono cliccabili, e la pila restante si ricompatta senza spazi vuoti.
