...
*   **Persistenza dei metadati (RFC vs. Draft):**
    *   **Per gli RFC pubblicati:** Una volta arricchiti, non vengono mai più riconsiderati. Il processo di `run_enrich()` utilizza un approccio puramente incrementale (`n["id"] not in result_nodes`): una volta risolti layer e working group, il dato è considerato definitivo. Questo garantisce performance elevate, ma significa che correzioni su Datatracker o modifiche ai `MANUAL_LAYER_OVERRIDES` non si propagheranno mai agli RFC già elaborati.
    *   **Per gli Internet-Draft (attivi/scaduti):** La gestione è duplice. Mentre il ciclo di vita (es. transizioni verso `dead` o `repl`) è monitorato periodicamente tramite `recheck_active_drafts()`, i metadati specifici (anno e abstract) sono soggetti a un secondo livello di "congelamento" operato da `draft_metadata_enricher.py`. Una volta processato un ID, questo viene marcato come "finito" e non viene più aggiornato nei run successivi, a meno di non utilizzare il flag `--force`, che tuttavia comporta la perdita dell'incrementalità ricostruendo l'intero dataset.
...
*   **Euristica di `compute_impact_scores()`:** Tale funzione non implementa un PageRank puro. Il termine di "authority boost" (proporzionale al numero grezzo di archi entranti, punto 8.1) viene sommato direttamente al rank ad ogni iterazione, rompendo l'invarianza per cui la somma dei rank dovrebbe restare costante tra le iterazioni. Si tratta di una scelta euristica dichiarata nel codice, non di un bug, ma tale comportamento va tenuto in considerazione qualora si volesse confrontare o riprodurre i punteggi con implementazioni di PageRank standard.
